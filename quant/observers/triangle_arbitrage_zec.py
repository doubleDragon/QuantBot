...
class TriangleArbitrage(BasicBot):
    """
    python -m quant.cli -mBitfinex_ZEC_USD,Bittrex_ZEC_BTC,Bitfinex_BTC_USD t-watch-triangle-arbitrage-zec -d
    ...
    def __init__(self, monitor_only=False):
        ...
        self.base_pair = 'Bitfinex_ZEC_USD'
        self.pair_1 = 'Bittrex_ZEC_BTC'
    ...
    def forward(self, depths):
        ...
        pair_2to1_zec_amount = pair2_bid_amount / pair1_bid_price

        max_trade_amount = config.zec_max_tx_volume
        hedge_zec_amount = min(base_pair_ask_amount, pair1_bid_amount)
        hedge_zec_amount = min(hedge_zec_amount, pair_2to1_zec_amount)
        hedge_zec_amount = min(max_trade_amount, hedge_zec_amount)

        if hedge_zec_amount < self.min_amount_zec:
            """bitfinex限制zec_usd最小可交易的zec order size为0.0001, bittrex限制为0.00000001, 所以取0.0001"""
            logging.info('hedge_zec_amount is too small! %s' % hedge_zec_amount)
            return

        hedge_btc_amount = hedge_zec_amount * pair1_bid_price
        if hedge_btc_amount < self.min_amount_btc:
            """bitfinex限制btc_usd最小可交易amount为0.005, 所以这里取0.005"""
            logging.info('hedge_btc_amount is too small! %s' % hedge_btc_amount)
            return

        """合成后的价格对标zec_usd, 以目前的bfx的价格设置小数位保留2位比较合适"""
        ...
        """zec_usd买，合成价格卖, 所以p_diff为合成价-base价"""
        p_diff = synthetic_bid_price - t_price
        profit = p_diff * hedge_zec_amount
        ...
        if profit > 0:
            ...
            logging.info(
                'r--buy %s zec @%s, sell BTC @synthetic: %s' % (self.base_pair, hedge_zec_amount, hedge_btc_amount))
            if profit < 10:
            ...
            if not self.monitor_only:
                self.brokers[self.base_pair].buy_limit(hedge_zec_amount, base_pair_ask_price)
                self.brokers[self.pair_1].sell_limit(hedge_zec_amount, pair1_bid_price)
    ...
    def reverse(self, depths):
        ...
        pair_2to1_zec_amount = pair2_ask_amount / pair1_ask_price
        # print(pair2_bid_amount, pair1_bid_price, pair_2to1_zec_amount)

        max_trade_amount = config.zec_max_tx_volume
        hedge_zec_amount = min(base_pair_bid_amount, pair1_ask_amount)
        hedge_zec_amount = min(hedge_zec_amount, pair_2to1_zec_amount)
        hedge_zec_amount = min(max_trade_amount, hedge_zec_amount)

        if hedge_zec_amount < self.min_amount_zec:
            """bfx限制zec最小订单数量为0.001"""
            logging.info('hedge_zec_amount is too small! %s' % hedge_zec_amount)
            return

        hedge_btc_amount = hedge_zec_amount * pair1_ask_price
        if hedge_btc_amount < self.min_amount_btc:
            """lq限制最小btc的total为0.0001, bfx的zec_usd交易订单限制amount为0.005"""
            logging.info('hedge_btc_amount is too small! %s' % hedge_btc_amount)
            return
        ...
        """zec_usd卖，合成价格买, 所以p_diff=base价-合成价"""
        p_diff = t_price - synthetic_ask_price

        profit = round(p_diff * hedge_zec_amount, self.precision)
        logging.info('profit=%s' % profit)
        ...
        if profit > 0:
            ...
            logging.info(
                'r--sell %s zec @%s, buy @synthetic: %s' % (self.base_pair, hedge_zec_amount, hedge_btc_amount))
            ...
            if current_time - self.last_trade < 10:
                ...
                return
            if not self.monitor_only:
                self.brokers[self.base_pair].sell_limit(hedge_zec_amount, base_pair_bid_price)
                self.brokers[self.pair_2].buy_limit(hedge_btc_amount, pair2_ask_price)
                self.brokers[self.pair_1].buy_limit(hedge_zec_amount, pair1_ask_price)
